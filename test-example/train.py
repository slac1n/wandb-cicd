...
def download_data():
    ...
    processed_data_at = wandb.use_artifact(f"{param.PROCESSED_DATA_AT}:latest")
...
def get_data(df, bs=4, img_size=180, augment=True):
    block = DataBlock(
        blocks=(ImageBlock, MaskBlock(codes=param.BDD_CLASSES)),
...
def log_predictions(learn):
    ...
    table = create_iou_table(samples, outputs, predictions, param.BDD_CLASSES)
...
def train(config, nrows=None):
    ...
    # good practice to inject param using sweeps
    config = wandb.config
